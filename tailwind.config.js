module.exports = {
	content: [
		'./src/pages/**/*.{js,ts,jsx,tsx}',
		'./src/components/**/*.{js,ts,jsx,tsx}',
	],
	theme: {
		extend: {
			maxWidth: {
				tela: '1920px',
			},
			spacing: {
				custom: '150px',
				navbartop: '0.8rem',
				navbarbottom: '0.8rem',
				book: '27%',
				personagem: '20%',
				academy: '6%',
				bookw: '300px',
				'1/2': '50%',
				'2/5': '40%',
				'1/4': '25%',
				'1/3': '33.33%',
				'3/4': '75%',
				'1/5': '20%',
				login: '650px',
			},
			fontSize: {
				xxs: '.65rem',
			},
			transitionDuration: {
				0: '0ms',
				2000: '2000ms',
			},
		},
		colors: {
			theme: '#000013',
			golden: '#D6A84C',
			lightGolden: '#D0B281',
			gray1: '#91A3AD',
			white2: '#FEF2E8',
			theme2: '#04042D',
			preto: '#000000',
		},
		backgroundImage: {
			gifTab: "url('/DualRing.svg')",
			bolinha: "url('/bolinha.png')",
			fumaca1: "url('/luz_livro.png')",
			fumaca2: "url('/luz_personagem.png')",
			fumaca3: "url('/luz_franquia.png')",
			fumaca4: "url('/luz_academy.png')",
			fumaca5: "url('/luz_celular.png')",
			fumaca6: "url('/luz_megafone.png')",
			bg1: "url('/background.jpg')",
			bg2: "url('/background2.jpg')",
			elem1: "url('/element1.png')",
			elem2: "url('/element2.png')",
			elem3: "url('/element3.png')",
			elem4: "url('/element4.png')",
			elem5: "url('/element5.png')",
			elem6: "url('/element6.png')",
			elem7: "url('/element7.png')",
			bg3: "url('/background3.jpg')",
			bg4: "url('/background4.jpg')",
			bg5: "url('/background5.jpg')",
			bg6: "url('/background6.jpg')",
			bg7: "url('/background7.jpg')",
			livro: "url('/book_-_livro.png')",
			celular: "url('/celular.png')",
			franquia: "url('/franqueado.png')",
			academy: "url('/academy.png')",
			personagem: "url('/personagem.png')",
			megafone: "url('/megafone.png')",
			smartbtech: "url('/smartbtech.png')",
			academyLogin: "url('/academyLogin.svg')",
			academyLoginCinza: "url('/academyLoginCinza.svg')",
			fundo: "url('/fundo.jpg')",
			academyLoginCinza270: "url('/academyLogin270Cinza.svg')",
			academyLogin270: "url('/academyLogin270.svg')",
			exchangeLogin: "url('/exchangeLogin.svg')",
			exchangeLoginCinza: "url('/exchangeLoginCinza.svg')",
			exchangeLogin270: "url('/exchangeLogin270.svg')",
			exchangeLoginCinza270: "url('/exchangeLogin270Cinza.svg')",
			bankLogin: "url('/bankLogin.svg')",
			bankLoginCinza: "url('/bankLoginCinza.svg')",
			bankLogin270: "url('/bankLogin270.svg')",
			bankLoginCinza270: "url('/bankLogin270Cinza.svg')",
			techLogin: "url('/techLogin.svg')",
			techLoginCinza: "url('/techLoginCinza.svg')",
			techLogin270: "url('/techLogin270.svg')",
			techLoginCinza270: "url('/techLogin270Cinza.svg')",
		},
		fontFamily: {
			gotham: 'Gotham HTF Medium',
			gothamBold: 'Gotham',
		},
		textUnderlineOffset: {
			10: '1.3rem',
		},
		gradientColorStops: {
			theme: '#000013',
			theme2: '#04042D',
		},
	},
	plugins: [],
};
